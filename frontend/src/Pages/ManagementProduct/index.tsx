...
const ManagementProduct = () => {
  ...
  const handleCreate = async () => {
    try {
      await axios.post('http://localhost:8080/api/management/products', {
            name: name,
  ...
  const handleUpdate = async (id: string, name: string, desciption: string, unitPrice: number, status: string) => {
    try {
      await axios
  ...
  return (
    <>
          ...
          <FormContainer>
            ...
            <SearchFormInput>
              ...
              <FormRow >
                ...
                name="unitPrice"
                value={unitPrice}
                onChange={(event: ChangeEvent<HTMLInputElement>) => setUnitPrice(parseFloat(event.target.value))}
      ...
      <ButtonContainer>
        <SubmitButton onClick={() => handleUpdate(identifier, name, description, unitPrice, status)} style={{backgroundColor: "#FF8C00"}} title={'ALTERAR'}/>
